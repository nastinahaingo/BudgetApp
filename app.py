...
def init_files():
    ...
    if not os.path.exists(BUDGET_DB):
        pd.DataFrame(columns=["id", "date", "description", "categorie", "type", "montant", "paiement", "auteur"]).to_csv(BUDGET_DB, index=False)
...
if not df.empty:
    ...
    with col1:
        ...
        if not df_dep.empty:
            fig_pie = px.pie(df_dep, values='montant', names='categorie', hole=0.4, title="Repartition", color_discrete_sequence=px.colors.sequential.Greys)
    ...
    with st.form("add_form", clear_on_submit=True):
        ...
        if st.form_submit_button("Enregistrer"):
            new_data = pd.DataFrame([[len(df), dt.strftime('%Y-%m-%d'), desc, cat, tp, mt, "Carte", st.session_state.user]], 
                                    columns=["id", "date", "description", "categorie", "type", "montant", "paiement", "auteur"])
...
if not df.empty:
    for i, row in df.sort_values("date", ascending=False).iterrows():
        st.markdown(f"""<div class="card"><b>{row['montant']:.2f} EUR</b> - {row['description']}<br>
        <span class="user-tag">{row['date'].strftime('%d/%m/%Y')} | {row['auteur']}</span></div>""", unsafe_allow_html=True)
